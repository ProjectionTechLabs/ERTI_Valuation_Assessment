...
function Login({ onRedirectToRegister, onLoginSuccess }) {
	...
	const handleSubmit = async () => {
		...
		if (!companyName.trim()) {
			setError("Enter your official company name");
			return;
		}
		...
		try {
			const res = await checkUser(companyName.trim());
			...
			<div className="hidden md:flex w-1/2 bg-linear-to-br from-blue-700 via-sky-700 to-cyan-700 text-white items-center justify-center p-10 relative overflow-hidden">
				...
				<div className="max-w-md text-center relative z-10">
					...
					<p className="text-lg opacity-90 mb-8 leading-relaxed">
						Use your registered mobile number and company name to jump straight
						into your valuation enhancement report.
					</p>
					<div className="grid grid-cols-3 gap-4 text-left">
						...
						<div className="rounded-2xl bg-white/10 border border-white/20 p-4">
							...
							<div className="text-sm opacity-80">Result journey</div>
						</div>
					</div>
				</div>
			</div>
				...
				<div className="bg-white shadow-xl rounded-3xl p-8 w-full max-w-md border border-blue-100">
					<div className="text-center mb-6">
						...
						<p className="text-sm text-gray-600 text-center mt-2">
							Enter your official company name and mobile number to continue
						</p>
					</div>
					...
					<button
						...
						disabled={!mobile || !companyName.trim() || loading}
					...
					<p className="text-sm text-gray-500 text-center mt-4">
						If your company is not found, you can{" "}
						<button
							type="button"
							onClick={() =>
								onRedirectToRegister({
